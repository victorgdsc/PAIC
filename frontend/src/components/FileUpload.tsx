...
const FileUpload: React.FC = () => {
  ...
  const handleExampleSelect = async (exampleFile: ExampleFile) => {
    try {
      ...
      const file = new File([blob], `${exampleFile.name}.csv`, {
        type: 'text/csv',
      });
      ...
      if (validateFile(file)) {
        ...
        await handleFileUpload(file, {
          numericColumns: exampleFile.numericColumns,
          columnMapping: exampleFile.columnMapping,
          exampleId: exampleFile.id
        });
      }
    } catch (err) {
      toast.error('Erro ao carregar o arquivo de exemplo');
      console.error(err);
    } finally {
